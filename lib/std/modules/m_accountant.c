...
private
float exchange_fee = 5.0; // In percent
...
void exchange4b(string currency1, string denomination2, string answer)
{
   ...
   if (amount2 > 0)
   {
      ...
      rate2 = to_float(MONEY_D->query_exchange_rate(currency2)) * MONEY_D->query_factor(denomination2);
...
void exchange4(string denomination1, int amount1, string currency2)
{
   ...
   if (MONEY_D->is_currency(currency2))
   {
      ...
      rate1 = to_float(MONEY_D->query_exchange_rate(currency1)) * MONEY_D->query_factor(denomination1);
...
void deposit2(string denomination)
{
   if (MONEY_D->is_denomination(denomination))
   {
      ...
      {
         write(capitalize(denomination) + " is not a denomination of " + deposit_currency + ".\n");
      }
   }
   else
   {
...
void withdraw3(string denomination, mixed answer)
{
   ...
   if (amount > 0)
   {
      ...
      {
         write("You don't have " + answer + " " + denomination + " in your account.\n");
      }
   }
}
...
{
   modal_pop();
   do_game_command("say Thanks for using " + bank_name + ", " + this_body()->short());
   if (was_busy)
   {
      was_busy--;
