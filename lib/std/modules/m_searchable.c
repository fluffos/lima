...
int query_need_specific_item()
{
   return need_spec_item;
}
...
int can_search_me(string sfor, object owith)
{
   ...
   if (once && searched)
      bad += 1;
   if (need_spec_item && sizeof(item) && member_array(sfor, item) != -1)
      bad += 1;
   if (sizeof(with) && member_array(file_name(owith), with) != -1)
      bad += 1;
   return !bad;
}
