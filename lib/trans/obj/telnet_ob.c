...
private
nosave int active_session = -1;
private
nosave string active_session_name;
private
nosave int actively_elsewhere;
private
nosave int notify_on_activity = 1;
private
nosave object owner;
private
nosave string control_char;
private
nosave int hide_all_output;

private
nomask string get_address(string);
private
nomask string get_unique_session_name(string);
private
nomask void open_socket(class connection);
private
nomask void recv_input_from(string, object, string);
private
nomask void close_session(string, object);
private
nomask void handle_command(string);
nomask void remove();

private
nomask void init_session(string session_name, string address)
{
...
private
nomask string get_address(string potential_mudname)
{
   mapping mudlist;
   string *matches;
...
private
nomask string get_unique_session_name(string session_name)
{
   int i = 1;
...
private
nomask void recv_input_from(string session, object sock, string input)
{
   string *info;
   int i;
   int session_num = -1;
...
private
nomask void handle_input(mixed input)
{
   if (input == -1)
   {
      remove();
...
private
nomask void handle_command(string cmd)
{
   string arg;
   class connection con;
   int i;
...
private
nomask void close_session(string ses, object sock)
{
   int i;
   int new_session = -1;
