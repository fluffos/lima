...
private
nomask string query_prompt()
{
   return sprintf("[%d of %d] mail: ", mailbox_ob->query_message_index() + 1, mailbox_ob->query_message_count());
}

private
void stdmail_cmd_replyall(mixed cmd_args)
{
...
private
void stdmail_cmd_reply(mixed cmd_args)
{
   if (cmd_args && !(cmd_args = to_int(cmd_args)))
   {
      write("Usage: r #\n");
...
void stdmail_cmd_help()
{
   ...
   if (wizardp(this_user()))
      output += "s [#] <filename>			Save message w/ header intact to file.\n"
...
private
void stdmail_cmd_forward(string cmd_args)
{
   mixed newto;
   int num;

   if (!cmd_args || (sscanf(cmd_args, "%d %s", num, newto) != 2 && (sscanf(cmd_args, "%s", newto) != 1)))
   {
...
private
void stdmail_cmd_save(string cmd_args)
{
   int num;
   string fname;

   if (!cmd_args || (sscanf(cmd_args, "%d %s", num, fname) != 2 && sscanf(cmd_args, "%s", fname) != 1))
   {
...
private
void stdmail_cmd_write(string cmd_args)
{
   int num;
   string fname;
...
private
nomask void mail_prompt(string input)
{
   string cmd_args;
