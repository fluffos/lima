...
int add_quest(string quest, int value, string base, string major_milestone)
{
...
int grant_points(object solver, string quest)
{
...
{
   return map(filter(keys(quests), ( : quests[$1][3] :)), ( : sprintf("%s (%s)", $1, quests[$1][3]) :));
}
...
string dump_final_goals()
{
   ...
   while (i--)
   {
      if (quests[keys[i]][3])
      {
         output += sprintf("%-17s-> %-2d pts, %d solves (%s)\n", keys[i], quests[keys[i]][1], quests[keys[i]][2],
                           quests[keys[i]][0]);
...
string show_quest(string quest)
{
   if (!quests[quest])
      return sprintf("%s: no such quest milestone.\n", quest);

   return sprintf("%-20s-> %-2d points, %d solves, home: %O", quest, quests[quest][1], quests[quest][2],
                  quests[quest][0]);
}
