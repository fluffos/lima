...
int stat_component(int STAT, int PART)
{
    ...
    stat = (constant_vector[STAT] * 10000 / 4 +
            trans_matrix[STAT][PART] * stats[PART] + 5000) /
           10000;
...
void refresh_stats()
{
    ...
    cur_con = stat_con + (trans_matrix[CON][STR] * adj_str +
                          ...
                          trans_matrix[CON][WIL] * adj_wil + 5000) /
                             10000;
    cur_wis = stat_wis + (trans_matrix[WIS][STR] * adj_str +
                          trans_matrix[WIS][AGI] * adj_agi +
                          trans_matrix[WIS][INT] * adj_int +
                          trans_matrix[WIS][WIL] * adj_wil + 5000) /
                             10000;
    cur_cha = stat_cha + (trans_matrix[CHA][STR] * adj_str +
                          trans_matrix[CHA][AGI] * adj_agi +
                          trans_matrix[CHA][INT] * adj_int +
                          trans_matrix[CHA][WIL] * adj_wil + 5000) /
                             10000;
    cur_man = stat_man + (trans_matrix[MAN][STR] * adj_str +
                          trans_matrix[MAN][AGI] * adj_agi +
                          trans_matrix[MAN][INT] * adj_int +
                          trans_matrix[MAN][WIL] * adj_wil + 5000) /
                             10000;
}
...
void recompute_derived()
{
    stat_con = (constant_vector[CON] * 10000 +
                trans_matrix[CON][STR] * (stat_str + mod_str) +
                trans_matrix[CON][AGI] * (stat_agi + mod_agi) +
                trans_matrix[CON][INT] * (stat_int + mod_int) +
                trans_matrix[CON][WIL] * (stat_wil + mod_wil) + 5000) /
               10000;
    stat_wis = (constant_vector[WIS] * 10000 +
                trans_matrix[WIS][STR] * (stat_str + mod_str) +
                trans_matrix[WIS][AGI] * (stat_agi + mod_agi) +
                trans_matrix[WIS][INT] * (stat_int + mod_int) +
                trans_matrix[WIS][WIL] * (stat_wil + mod_wil) + 5000) /
               10000;
    stat_cha = (constant_vector[CHA] * 10000 +
                trans_matrix[CHA][STR] * (stat_str + mod_str) +
                trans_matrix[CHA][AGI] * (stat_agi + mod_agi) +
                trans_matrix[CHA][INT] * (stat_int + mod_int) +
                trans_matrix[CHA][WIL] * (stat_wil + mod_wil) + 5000) /
               10000;
    stat_man = (constant_vector[MAN] * 10000 +
                trans_matrix[MAN][STR] * (stat_str + mod_str) +
                trans_matrix[MAN][AGI] * (stat_agi + mod_agi) +
                trans_matrix[MAN][INT] * (stat_int + mod_int) +
