...
private
nosave int escape_hit;
private
nosave string matchstr = "";
private
nosave mapping bindings;
private
nosave function default_func;

private
nomask void handle_one_char(int);
...
private
nomask int match_filter(string binding)
{
   return binding == matchstr;
}

private
nomask int partial_match_filter(string binding)
{
   return strsrch(binding, matchstr) == 0;
}

private
nomask void handle_one_char(int c)
{
