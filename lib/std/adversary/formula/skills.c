...
int chance_to_hit(object weapon, object target)
{
  ...
  int skill_contrib = strsrch(attack_skill, "combat/guns") != -1 ? (this_object()->query_dex() / TO_HIT_STAT_IMPACT) : (this_object()->query_str() / TO_HIT_STAT_IMPACT);
  int attack_value;
  int defend_value;
  int attack_mod = 1;
  int defend_mod = 1;
...
int defend_chance(object weapon, object target)
{
  ...
  if (combat_debug)
    TBUG("defend," + defend_skill + "," + defend_value + ",attack," + attack_skill + "," + attack_value + " skill impact: " + skill_impact + " res: " + res + "\n");
  return res;
}
...
int disarm_chance(object target)
{
  ...
  int disarm_prevention =
      (target ? target->query_anti_disarm() : 0) +
...
{
  string attack_skill = weapon ? weapon->query_skill_used() : "combat/melee/unarmed";
  int skill_contrib = strsrch(attack_skill, "combat/guns") != -1 ? (this_object()->query_dex() / DAMAGE_STAT_IMPACT) : (this_object()->query_str() / DAMAGE_STAT_IMPACT);

  //If the weapon is set to restricted (happens when wielded in /std/adversary/wield/limbs)
  //we reduce the damage by 90% (you suck!).
  if (combat_debug)
    TBUG("Damage parts: Weapon class: " + weapon->query_weapon_class() + " Skill rank: " + cached_skill_rank(attack_skill) + " attack_skill: " + attack_skill + " dmg bonus: " + query_damage_bonus(target) + " skill_contrib: " + skill_contrib);
  return cached_skill_rank(attack_skill) + random(weapon->query_weapon_class()) + 1 + query_damage_bonus(target) + skill_contrib;
}
