...
private
nomask void mail_add_request(string, mixed array);

nosave private mapping mail_requests = ([]);

private
nomask void handle_mail(string mudname, object socket, mixed *message)
{
   string *errors;
...
private
nomask void handle_mail_ack(string mudname, object socket, mixed *message)
{
   IMAIL_D->receive_ack(message[1]);
}

protected
nomask int mail_has_outgoing(string mudname)
{
   return sizeof(mail_requests[mudname]) != 0;
}

protected
nomask int mail_send_outgoing(string mudname, object socket)
{
...
private
nomask void mail_add_request(string mudname, mixed *request)
{
...
public
nomask void send_mail_message_to_mud(mixed *packet, string mudname)
{
