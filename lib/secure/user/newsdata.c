...
void save_me();
private
string *subscribed_groups();
private
int check_subscribed(string group);

private
nomask mapping news_data;
private
nomask int threading_toggled = 1;
...
{
   news_data[group] = sprintf("%c%s", news_data[group][0], set_add_range("", 1, NEWS_D->get_group_last_id(group)));
   save_me();
}
...
nomask void add_news_id_read(string group, int id)
{
   ...
   {
      news_data[group] = sprintf("%c%s", news_data[group][0], set_remove(news_data[group][1..], 0));
   }
   news_data[group] = sprintf("%c%s", news_data[group][0], set_add(news_data[group][1..], id));
   save_me();
}
...
nomask void unsubscribe_group(string group)
{
   ...
   {
      news_data[group] = sprintf("#%s", news_data[group][1..]);
   }
   save_me();
}
...
nomask string *subscribed_groups()
{
   ...
   return filter(groups, ( : check_subscribed:));
}
