...
private
mapping relation_aliases = ([]); /* Sometimes "in" and "on" etc mean the
                                  * same thing, for example on the bed. */
private
string default_relation; /* The default relation used for this
...
private
nomask int valid_relation(string relation)
{
...
mixed *make_objects_if_needed()
{
  ...
  foreach (string relation, class relation_data object_data in relations)
  {
    ...
    foreach (string file, mixed parameters in object_data->create_on_reset)
    {
      ...
      {
        matches = filter(object_data->contents, (
                                                    : cannonical_form($1) == $(file)
                                                    :));
      }
                   ...
                   mixed other)
{
  ...
  if (environment_can_hear() && (env = environment()) && (!arrayp(exclude) || member_array(env, exclude) == -1))
  {
    env->receive_inside_msg(msg, arrayp(exclude) ? exclude + ({this_object()}) : ({this_object()}),
                            message_type, other);
  }
}
...
string stat_me()
{
  return "Prepositions: " + implode(keys(relations), ",") + "\n" +
         "It contains:\n" + show_contents() + "\n" +
         ::stat_me();
}
