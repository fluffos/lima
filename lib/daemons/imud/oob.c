...
private
nomask int oob_send(class oob_info info, mixed *message)
{
   DBBUG(message);
...
private
nomask void oob_error(class oob_info info, string errcode, string errmsg, mixed *errpacket)
{
...
protected
nomask void oob_svc_send(object socket, mixed *message)
{
...
protected
nomask void oob_svc_error(object socket, string errcode, string errmsg, mixed *errpacket)
{
   class oob_info info = oob_socket_map[socket];
...
private
nomask int oob_has_outgoing(string remote_mudname)
{
   return (mail_has_outgoing(remote_mudname) || file_has_outgoing(remote_mudname));
}
...
private
nomask void oob_callback_read(object socket, mixed *message)
{
   class oob_info info = oob_socket_map[socket];
   function f_request;
   function f_reply;
   ...
   */
   ...
   case "oob-begin":
      ...
      else if (info->state == OOB_STATE_WAIT_BEGIN)
      {
         ...
         {
            LOG_D->log(LOG_I3_ERROR, sprintf("%s failed the authentication test\n", message[1]));
            oob_close(info);
         }
         else
         {
...
private
nomask void oob_callback_close(object socket)
{
   class oob_info info = oob_socket_map[socket];
...
private
nomask void oob_open_connection(string mudname, int auth_type, int auth_token)
{
...
varargs nomask void oob_initiate_connection(string mudname, function fail_func)
{
   ...
   else
   {
      ...
      call_out(( : oob_open_connection, mudname, OOB_AUTH_TYPE_NONE, 0 :), OOB_OPEN_DELAY);
   }
}

protected
nomask void oob_handle_auth_mud_reply(string mudname, int session_key)
{
   class oob_info info = oob_mudname_map[mudname];
...
protected
nomask void oob_register_requests(mapping requests)
{
   oob_requests += requests;
}
protected
nomask void oob_register_replies(mapping replies)
{
   oob_replies += replies;
}
...
protected
nomask void oob_startup()
{
   string err;

   err = catch (oob_socket = new (SOCKET, SKT_STYLE_LISTEN_M, PORT_I3_TCP_OOB,
                                  (
                                      : oob_callback_read:),
                                  (
                                      : oob_callback_close:)));
