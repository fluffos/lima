...
varargs void tell_from_inside(mixed ob, string what, int msg_type, mixed exclude)
{
   ob->receive_inside_msg(what, exclude, msg_type | INSIDE_MSG);
}

varargs void tell_from_outside(mixed ob, string what, int msg_type, mixed exclude)
{
   ob->receive_outside_msg(what, exclude, msg_type | OUTSIDE_MSG);
}

varargs void tell_environment(mixed ob, string what, int msg_type, mixed exclude)
{
   object env;
...
varargs void tell_user(string username, string what, int msg_type)
{
   object who;
