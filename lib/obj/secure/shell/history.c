...
private
nosave int buffer_size = DEFAULT_HISTORY_BUFFER_SIZE;
private
nosave int array_index;
private
nosave int command_number;
...
protected
string history_command(mixed input)
{
   int cmdnumber;
   ...
   default:
      if (sscanf(input[1..], "%d", cmdnumber))
      {
         ...
         if (cmdnumber < 1 || cmdnumber > command_number || (input = get_nth_item(cmdnumber)) == -1)
         {
