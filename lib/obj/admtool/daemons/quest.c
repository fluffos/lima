...
private
nomask void get_milestone(string quest_name, int value, string file, string s)
{
...
private
nomask void get_master(string quest_name, int value, string master)
{
   ...
   modal_simple((
                    : get_milestone, quest_name, value, file:),
                "If this is a quest endpoint, give a one line description of the quest for\nplayers, or else hit enter "
                "(and nothing else!): ");
}

private
nomask void get_value(string quest_name, string pts_as_string)
{
   int value = to_int(pts_as_string);
   printf("Quest %s' value is set at %d points.\n", quest_name, value);
   modal_simple(( : get_master, quest_name, value:), "What file can dole out these points? ");
}

private
nomask void add_quest(string name)
{
   ...
   modal_simple(( : get_value, name:), "How many points is this quest item worth? ");
}

private
nomask void remove_quest(string name)
{
