...
void add_cache_entry(string mudname, string username, string visname, int gender);
string get_visname(string mudname, string username);
string canon_mudname(string mudname);

protected
nomask int query_chanlist_id()
{
   return chanlist_id;
}

nomask void channel_rcv_data(string channel_name, string sender_name, string type, mixed data)
{
   if (filter_msg)
   {
      filter_msg = 0;
...
protected
nomask void rcv_chanlist_reply(string orig_mud, string orig_user, string targ_user, mixed *message)
{
   string channel_name;
   mixed channel_data;
   int i;
...
protected
nomask void rcv_chan_who_req(string orig_mud, string orig_user, string targ_user, mixed *message)
{
   object *listeners = CHANNEL_D->query_listeners("imud_" + message[0]);

   if (!listeners)
   {
      return_error(orig_mud, orig_user, "unk-channel", sprintf("Never heard of '%s'", message[0]));
   }
   else
   {
      listeners -= ({0, this_object()});
      listeners = filter(listeners, ( : userp($1) :));
      send_to_user("chan-who-reply", orig_mud, orig_user, ({message[0], listeners->query_body()->query_name()}));
   }
}

protected
nomask void rcv_chan_who_reply(string orig_mud, string orig_user, string targ_user, mixed *message)
{
   object p;
   ...
   if (!p)
   {
      return_error(orig_mud, orig_user, "unk-user", sprintf("'%s' is unknown", targ_user));
   }
   else
   {
      tell(p, sprintf("%s@%s listeners: ", message[0], orig_mud) + implode(message[1], ", ") + ".\n");
   }
}
...
protected
nomask void rcv_channel_m(string orig_mud, string orig_user, string targ_user, mixed *message)
{
   if (orig_mud == mud_name())
      return;

   filter_msg = 1;
   CHANNEL_D->deliver_tell("imud_" + message[0], sprintf("%s@%s", message[1], orig_mud), message[2]);
}

protected
nomask void rcv_channel_e(string orig_mud, string orig_user, string targ_user, mixed *message)
{
   if (orig_mud == mud_name())
      return;
   ...
   if (sizeof(message) != 3)
   {
      return_error(orig_mud, orig_user, "bad-pkt", "Bad channel-e format. Report to your admin.");
      return;
   }
   ...
   */
   CHANNEL_D->deliver_channel("imud_" + message[0],
                              replace_string(message[2], "$N", sprintf("%s@%s", message[1], orig_mud)));
}

protected
nomask void rcv_channel_t(string orig_mud, string orig_user, string targ_user, mixed *message)
{
   ...
   if (mud_name() == message[1])
   {
      ...
      if (targ_ob)
      {
         ...
         CHANNEL_D->deliver_soul("imud_" + message[0], ({({targ_ob}), ({targ_mess, all_mess})}));
         return;
      }
   }
   else
...
protected
nomask void rcv_chan_user_req(string orig_mud, string orig_user, string target_user, mixed *message)
{
   object p;
   ...
   if (!p)
   {
      return_error(orig_mud, orig_user, "unk-user", sprintf("'%s' is unknown", message[0]));
   }
   else
   {
...
protected
nomask void chan_startup()
{
   CHANNEL_D->register_channels(map_array(listened_channels, ( : "imud_" + $1:)));
}

protected
nomask void chan_shutdown()
{
...
{
   foreach (string str in listened_channels)
...
nomask string *query_listened_channels()
{
   return copy(listened_channels);
}
