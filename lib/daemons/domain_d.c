...
int spawn_allowed(string basename)
{
    ...
    instances = sizeof(filter_array(objects(), (
                                                   : base_name($1) == $(basename)
                                                   :))) -
                1;
    ...
    if (spawn)
        CHANNEL_D->deliver_channel("domains", "[" + capitalize(spawn_control[basename]["domain"]) + "]: " +
                                                  "Allowed " + mob_name(basename) + " (" + (instances + 1) + "/" + spawn_control[basename]["max"] + ")");
    /*
        (spawn ?
