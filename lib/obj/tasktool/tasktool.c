...
{
   string t_id = TASK_D->add_task(our_task, title, desc, this_user()->query_userid());
...
class command_info *module_commands()
{
   ...
   if (our_task != 0)
   {
      ...
      ret += ({new (class command_info, desc : t[1])});
   }
   ...
   foreach (mixed *task in TASK_D->query_tasks(our_task))
   {
      ...
      if (key)
      {
         ...
         ret += ({new (class command_info, key
                       : key, desc
                       : name, action
                       : (
                             : clone_object, file, task_id:),
                         status
                       : status)});
      }
...
void create(string task_id)
{
   if (clonep())
   {
      ...
      our_defaults += ({new (class command_info, key
                             : "q", desc
                             : "quit", action
                             : (
                                 : do_quit:)),
      ...
      if (file_name(previous_object()) != CMD_OB_TASKTOOL && base_name(previous_object()) != TASKTOOL)
      {
