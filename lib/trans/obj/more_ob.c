...
nomask void do_more(mixed arg)
{
   ...
   if (arg)
      ...
      case 'd': // Toggle Direction
         direction = -direction;
         write("more: now scanning " + (direction == 1 ? "forward" : "backward") + "\n");
         return;
      case 'n': // Next file if any
         ...
         if (file_index < 0 || file_index >= sizeof(file_list))
         {
            write("more: no more files " + (direction == 1 ? "after" : "preceding") + "\"" + file_list[file_index - 1] +
                  "\"\n");
            file_index -= direction;
            return;
         }
      ...
      case 'b':
         ...
         break;
      case 'e':
         line_index = (sizeof(lines) - chunk_size >= 0 ? sizeof(lines) - chunk_size : 0);
         break;
      case 'a':
         break;
      default: // Next chunk
   ...
   while (1)
   {
      if (!lines)
      {
         ...
         {
            write("more: file \"" + file_list[file_index] + "\" contains nothing\n");
            if (next_file())
               break;
            continue;
         }
      ...
      if (current_search)
      {
         for (; line_index < sizeof(lines) && line_index >= 0; line_index += direction)
            if (regexp(lines[line_index], current_search))
               break;
         if (line_index < 0 || line_index >= sizeof(lines))
         {
            write("more: \"" + current_search + "\" not found" +
                  (file_list ? " in \"" + file_list[file_index] + "\"\n" : "\n"));
            if (next_file())
            {
...
void create(int kind, mixed arg, int c, function continuation, int of)
{
   set_privilege(1);
