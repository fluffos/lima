...
private
nomask string get_level(mixed m)
{
   return adminp(m) ? "admin" : wizardp(m) ? "wizard" : "player";
}

private
nomask string get_idle(object o)
{
   int i;
...
class finger get_finger_data(string userid)
{
   ...
   // Modified by Aziz to add marriage functions.
   ...
   result = new (class finger,
                 // Modified again (spouse: info[4])
                 // ### the visname should be queried from somewhere...
                 visname
                 : capitalize(userid), real_name
                 : info[0], email
                 : info[1], nickname
                 : info[2], home_page
                 : info[3], spouse
   ...
   if (!this_user() || (userid != this_user()->query_userid() && !check_privilege(1)))
   {
      ...
      if (result.home_page && result.home_page[0] == '#')
         result.home_page = 0;
#else
      result.email = result.real_name = result.home_page = 0;
#endif
   }
   ...
   return implode(u,
                  (
                      : $1 + sprintf("%-15s%-12s%-5s%s\n",
                                     ...
                                     get_level($2), get_idle($2), query_ip_name($2))
                      :),
                  sprintf("\n[%s] %d user%s presently connected (%s)\n%s\n", mud_name(), sizeof(u),
                          sizeof(u) == 1 ? "" : "s", ctime(time()), repeat_string("-", 75)));
}
...
string show_big_html_finger()
{
   ...
   retval = sprintf(
       "<META HTTP-EQUIV=\"Refresh\" CONTENT=30><center><TABLE BORDER=5><caption><font size=+2>%s</font></caption>"
   ...
   foreach (object user in u)
   {
      retval += sprintf("<TR><TD>%s</TD><TD>%s</TD><TD>%s</TD><TD>%s</TD></TR>",
                        find_body(user->query_userid())
                            ? sprintf("<font size=+2><a href=http://%s:%d/cgi/who.c?=%s>%s</a></font>", __HOST__,
                                      PORT_HTTP, user->query_userid(), capitalize(user->query_userid()))
                            : "Login",
                        get_level(user), get_idle(user), query_ip_name(user));
   }
