...
private
nomask string get_fname(int message_key)
{
...
private
nomask void save_msg(int message_key, class mail_msg msg)
{
   saved_msg = msg;
...
private
nomask class mail_msg restore_msg(int message_key)
{
   class mail_msg msg;
...
private
nomask void deliver_mail(int message_key, string who)
{
...
private
nomask mixed clean_addresses(string *list)
{
   string user, mudname;
   mixed mudinf;
   string *local_recips = ({});
...
varargs nomask string *send_mail(string Sender, string Subject, mixed Body, string *To_list, string *Cc_list,
                                 ...
                                 int send_time)
{
   ...
   if (!check_previous_privilege(1) && (!this_user() || this_user()->query_userid() != Sender))
   ...
   */
   ...
   if (previous_object() != find_object(IMAIL_D) && sizeof(recip_list) != sizeof(local_recip_list))
   {
...
nomask mixed get_one_message(int message_key)
{
   ...
   {
      LOG_D->log(LOG_MAIL_ERROR, sprintf("%s lost message #%d", this_user()->query_userid(), message_key));
      return 0;
   }
...
nomask void delete_mail(int message_key, string user)
{
   ...
   if (base_name(previous_object()) != MAILBOX || previous_object()->query_owner() != user)
...
private
nomask void process_message(string fname)
{
   int message_key;
   class mail_msg msg;
