...
void set_board_desc(string what)
{
   boarddesc = what;
}
...
void reset()
{
   if (!random(4))
   {
      tell_environment(this_object(),
                       "A small gnome appears and secures some " + "notes on the board that were loose.\n");
...
mixed direct_note_str_on_obj(string str, object ob)
{
   return 1;
}

mixed direct_read_str_on_obj(string str, object ob)
{
   return 1;
}

mixed direct_remove_str_from_obj(string str, object ob)
{
   return 1;
}
