...
void prompt_user(string *asContents, string sPath, string sOrigFile, string sSelection);
...
be no backups to restore.  Exiting...\n",
   ...
   asBakDirContents = get_dir(evaluate_path((sPath ? sPath : ".") + "/bak/" + sModFName + "*"));
   ...
   {
      sMsg =
          sprintf("Sorry!  The file %s has not been previously backed up.\n", (sPath ? sPath : ".") + "/" + sFilename);
      outf("%-=75s", sMsg);
      return;
   }
   else
...
void prompt_user(string *asContents, string sPath, string sOrigFile, string sSelection)
{
   ...
   {
      // Validate input.
      if (!sSelection || sSelection == "q" || sSelection == "x")
      {
      ...
      if (!to_int(sSelection) || (to_int(sSelection) > sizeof(asContents) || to_int(sSelection) < 0))
      {
      ...
      write("Restoring \"" + asContents[to_int(sSelection) - 1] + "\" to \"" + sPath + "/" + sOrigFile + "\"...\n");

      if (!sFileContents = read_file(evaluate_path(sPath + "/bak/" + asContents[to_int(sSelection) - 1])))
      {
