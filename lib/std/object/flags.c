...
*/
private
nosave mapping non_persist_flags;
private
...
// called.
private
void set_flags(int which, int state)
{
   int set_key;
   class flag_set_info set_info;
   int value;
...
varargs nomask void configure_set(int set_key, int is_non_persistent, function change_func)
{
   if (!flag_sets)
      init_vars();
