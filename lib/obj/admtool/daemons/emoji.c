...
nomask void show_users_emojis(string user)
{
  ...
  emojis = filter(EMOJI_D->get_emoji_data(),
                  (
  ...
  if (!sizeof(emojis))
    ...
    printf("%s belongs to: %s.\n", user,
           implode(sort_array(keys(emojis), 1), ", "));
}
...
{
  input_two_args("UTF8 Char? ",
                 "Replacement (default empty)? ",
                 (
                     : add_data_to_emoji2, emoji:));
}
...
nomask class command_info *module_commands()
{
  return ({new (class command_info,
           ...
           new (class command_info,
                ...
                : "[emoji]",
                  desc
                : "show an emoji's data",
                  args
                : ({"Show data for which emoji? "}),
                  action
                : (
                    : show_emoji_data:)),
           new (class command_info,
           ...
           new (class command_info,
                ...
                : "[emoji]",
                  desc
                : "remove data from a emoji",
                  who
                : "[Mudlib:]",
                  args
                : ({"Remove from which emoji? "}),
                  action
                : (
