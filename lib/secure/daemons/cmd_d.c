...
void parse_verb_defs(string dir, string filecontents)
{
   ...
   foreach (line in lines)
   {
      ...
      // ### gross.
      ((class proto_info)pathmap[cmd])->proto_string = replace_string(
          replace_string(
              replace_string(
                  replace_string(replace_string(replace_string(implode(words, " "), "num", "number"), "str", "string"),
                                 "obj", "object"),
                  "fname", "filename"),
              "*", "(s)"),
          "cfile", "file");
...
varargs mixed find_cmd_in_path(string cmd, string *path)
{
   ...
   foreach (dir in path)
   {
      ...
      if (!cmd_info[dir])
         continue;
      if (member_array(cmd, cmd_info[dir]) != -1 && is_file(dir + cmd + ".c") &&o = load_object(dir + cmd))
      {
         if (!is_command(o))
            return -1;
         return ({o, dir, cmd});
...
mixed smart_arg_parsing(mixed argv, string *path, string *implode_info)
{
   ...
   if (member_array('/', cmd_name) != -1)
   {
      ...
      {
      case 1:
         if ((cmd_obj = load_object(matches[0])) && is_command(cmd_obj))
         {
...
private
mixed parse_arg(int this_arg, mixed argv)
{
   int hits;
