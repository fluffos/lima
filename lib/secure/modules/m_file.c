...
private
class file_info find_file(string fn)
{
   if (fn)
   {
...
private
void flush_buffer(class file_info fi)
{
   if (fi.bchanged)
   {
...
private
void refill_buffer(class file_info fi)
{
   flush_buffer(fi);
...
void file_open(string fn, string mode)
{
   ...
   open_files[fn] = cur_file = new (class file_info, fname : fn, pos : p);
}
