...
private
nomask void show_guild(string name)
{
   string str, temp;
   string *ary;
   int i;
...
private
nomask void add_guild(string name)
{
...
private
nomask void remove_guild(string name)
{
...
private
nomask void set_title2(string name, string title)
{
   GUILD_D->set_guild_title(name, title);
}

private
nomask void set_title(string name)
{
   modal_simple(( : set_title2, name:), "Title? ");
}

private
nomask void send_prospectus(string name, string *lines)
{
   GUILD_D->set_guild_prospectus(name, implode(lines, " "));
}

private
nomask void prospectus(string name)
{
...
private
nomask void ban(string name, string guild)
{
   ...
   GUILD_D->set_guild_banned(name, GUILD_D->query_guild_banned(name) + ({guild}));
}

private
nomask void prereq(string name, string guild)
{
   ...
   GUILD_D->set_guild_prereq(name, GUILD_D->query_guild_prereq(name) + ({guild}));
}

private
nomask void ally(string name, string guild)
{
   ...
   GUILD_D->set_guild_allies(name, GUILD_D->query_guild_allies(name) + ({guild}));
}

private
nomask void exclude(string name, string attr)
{
   ...
   GUILD_D->set_guild_exclusive(name, GUILD_D->query_guild_exclusive(name) + ({attr}));
}

private
nomask void attrib(string name, string attr)
{
   ...
   GUILD_D->set_guild_attributes(name, GUILD_D->query_guild_attributes(name) + ({attr}));
}

private
nomask void begone(string name, string level)
{
   GUILD_D->set_guild_begone(name, to_int(level));
}

private
nomask void suspend(string name, string level)
{
   GUILD_D->set_guild_suspend(name, to_int(level));
}

private
nomask void see_secret(string name)
{
   if (GUILD_D->query_guild_sees_secret(name))
   {
...
private
nomask void is_secret(string name)
{
   if (GUILD_D->query_guild_is_secret(name))
   {
...
private
nomask void need_all(string name)
{
   if (GUILD_D->query_guild_need_all(name))
   {
...
nomask class command_info *module_commands()
{
   return ({
       ...
       new (class command_info, key
            : "b", proto
            : "[name] [guild to be banned]", desc
            : "Add to a guild's banned list", who
            : "[Mudlib]", priv
            : "Mudlib", args
            : ({"What name? ", "Which guild? "}), action
            : (
                : ban:)),
       new (class command_info, key
            : "P", proto
            : "[name] [guild to be needed]", desc
            : "Add to a guild's prereq list", who
            : "[Mudlib]", priv
            : "Mudlib", args
            : ({"What name? ", "Which guild? "}), action
            : (
       ...
       new (class command_info, key
            : "A", proto
            : "[name] [ally]", desc
            : "Add to a guild's ally list", who
            : "[Mudlib]", priv
            : "Mudlib", args
            : ({"What name? ", "Which guild? "}), action
            : (
       ...
       new (class command_info, key
            : "e", proto
            : "[name] [attribute]", desc
            : "Add to guild's exclude list", who
            : "[Mudlib]", priv
            : "Mudlib", args
            : ({"What name? ", "Which attribute? "}), action
            : (
                : exclude:)),
       new (class command_info, key
            : "a", proto
            : "[name] [attribute]", desc
            : "Add to guild's attributes", who
            : "[Mudlib]", priv
            : "Mudlib", args
            : ({"What name? ", "Which attribute? "}), action
            : (
       ...
       new (class command_info, key
            : "B", proto
            : "[name] [level]", desc
            : "Set a guild's begone level", who
            : "[Mudlib]", priv
            : "Mudlib", args
            : ({"What name? ", "Begone level? "}), action
            : (
                : begone:)),
       new (class command_info, key
            : "s", proto
            : "[name] [level]", desc
            : "Set a guild's suspend level", who
            : "[Mudlib]", priv
            : "Mudlib", args
            : ({"What name? ", "Suspend level? "}), action
            : (
                : suspend:)),
       new (class command_info, key
            : "S", proto
            : "[name]", desc
            : "Toggle a guild's sees_secret flag", who
            : "[Mudlib]", priv
            : "Mudlib", args
            : ({"What name? "}), action
            : (
       ...
       new (class command_info, key
            : "i", proto
            : "[name]", desc
            : "Toggle a guild's is_secret flag", who
            : "[Mudlib]", priv
            : "Mudlib", args
            : ({"What name? "}), action
            : (
       ...
       new (class command_info, key
            : "n", proto
            : "[name]", desc
            : "Toggle a guild's need_all flag", who
            : "[Mudlib]", priv
            : "Mudlib", args
            : ({"What name? "}), action
            : (
