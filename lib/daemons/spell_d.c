...
private
int valid_spell_name(string spell_name)
{
   return stringp(spell_name) && regexp(spell_name, "^[a-zA-Z0-9 '-]+$");
}

private
void build_spell_table()
{
   string *bad_dirs = ({});
...
private
nomask string find_spell_obname(string spell)
{
...
varargs mixed check_valid_target(string spell, object target, mixed has_reagent)
{
   string spell_obname;
...
varargs mixed check_valid_reagent(string spell, object reagent, mixed has_target)
{
   string spell_obname;
