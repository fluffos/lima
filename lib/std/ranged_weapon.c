...
int query_loaded() { return query_charges() > 0; }

// Is it loaded ?
int query_time_to_load() { return time_to_load; }
void set_time_to_load(int t) { time_to_load = t; }
...
{
  return ammo_ob ? "Ammo-wise the weapon is " + evaluate_ammo(ammo_ob) + " with " + query_ammo_type() + " rounds, "
