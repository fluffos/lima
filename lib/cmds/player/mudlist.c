...
*/
#define INFO_DRIVERTYPE ({({0, 2}), ({11, 18}), ({8, 8}), ({7, 20}), ({5, 18})})

#define INFO_MUDTYPE ({({0, 2}), ({11, 18}), ({8, 8}), ({1, 15}), ({2, 5}), ({5, 18})})

#define INFO_DEFAULT ({({0, 2}), ({11, 15}), ({1, 15}), ({2, 5}), ({5, 12}), ({9, 17})})

private
nosave string *headers = ({
    "Up",                  // 0
    "Address",             // 1
    "", /* port */         // 2
    "", /* TCP OOB port */ // 3
...
void main(mixed *arg, mapping flags)
{
   ...
   foreach (match in sort_array(matches, 1))
   {
      ...
      line = sprintf(format, mudinfo[0] == -1 ? "U" : "D", match[0..info [1][1] - 1],
                     map_array(info[2..], (
                                              : ($(mudinfo)[$1[0]] + "")[0.. $1[1] - 1]
                                              :))...);
      if (search_str)
