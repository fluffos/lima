...
void do_go_obj(object ob, mixed s)
{
   ...
   if (ob == this_object() && stringp(s) && (member_array(s, query_exit_directions(1)) != -1))
   {
      do_go_somewhere(s);
      return;
   }
   if (!stringp(s))
      this_body()->move_to(ob->query_default_destination(), 0, ob->query_default_exit_msg(),
   ...
   else
   {
      ...
      this_body()->move_to(dest, s, ob->query_prep_exit_msg(s), ob->query_prep_enter_msg(s));
   }
}
...
void do_go_somewhere(string dir)
{
   ...
   if (!this_body()->query_driving_vehicle())
      this_body()->move_to(query_exit_destination(dir, base), dir, query_exit_msg(dir), query_enter_msg(dir));
   else
      environment(this_body())
          ->move_to(query_exit_destination(dir, base), dir, query_exit_msg(dir), query_enter_msg(dir));
}
