...
void main(mixed *arg)
{
   ...
   foreach (string target in targets)
   {
      ...
      else if (member_array(target, bodies) != -1)
      {
         ...
         outf("%-13s%-30s[%-s]\n", capitalize(curr_body->query_userid()), file_name(environment(curr_body)),
              environment(curr_body)->short());
      }
      else
      {
         outf("%-13s%-30s[%-s]\n", capitalize(lower_case(target)), "Not Currently On ", "Physical Storage");
      }
   }
}
