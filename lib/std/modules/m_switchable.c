...
int is_on;
private
string switch_on_msg = "$N $vswitch a $o on.";
private
string switch_off_msg = "$N $vswitch a $o off.";
...
void turn_on()
{
   ...
   if (!tmp)
      tmp = "You can't seem to turn it on.\n";
   if (stringp(tmp))
      write(tmp);
   else
      this_body()->simple_action(switch_on_msg, this_object());
}
...
void turn_off()
{
   ...
   if (!tmp)
      tmp = "You can't seem to turn it off.\n";
   if (stringp(tmp))
      write(tmp);
   else
      this_body()->simple_action(switch_off_msg, this_object());
}
