...
private
nosave mixed def_dest = 0;
private
nosave mixed def_exit_msg = 0;
private
nosave mixed def_enter_msg = 0;
private
nosave mixed def_check = 0;
private
nosave mixed default_desc = 0;
private
nosave mapping prep_dest = ([]);
private
nosave mapping prep_exit_msg = ([]);
private
nosave mapping prep_enter_msg = ([]);
private
nosave mapping prep_check = ([]);
private
nosave mapping prep_desc = ([]);
private
nosave string go_verb = "go";
private
nosave string base = 0;
private
nosave mapping prep_aliases = ([]);
...
{
   if (stringp(prep))
      return "$N $v" + query_go_method() + " " + prep + " " + this_object()->the_short() + ".";
   else
      return "$N $v" + query_go_method() + " " + this_object()->the_short() + ".";
}
...
mixed direct_go_word_obj(string prep)
{
   ...
   if (undefinedp(prep_dest[prep]))
      return "You can't " + query_go_method() + " " + prep + " that, only " + format_list(keys(prep_dest), "or") + ".";
...
mixed direct_go_obj()
{
   ...
   if (def_dest == 0)
   {
      ...
      default:
         return "Do you want to " + query_go_method() + " " + format_list(keys(prep_dest), "or") + " that?";
      }
   }
...
void propogate_exits_up()
{
   ...
   foreach (dir in dirs)
   {
      if (member_array(dir, environment(this_object())->query_exit_directions()) == -1)
