...
void syslog(string text)
{
   unguarded(1,
             (
                 : write_file, ACCESS_LOG, (this_user() ? this_user()->query_userid() : "<Server>") + ": " + text + "\n"
                 :));
}
...
private
mixed *walk_path(string file, int write)
{
   string *path;
   mixed *result;
   mixed *tmp;
...
private
int valid_domain_name(string name)
{
...
private
string get_priv_owner(string priv)
{
   int n;
   if (!stringp(priv))
      return 0;
