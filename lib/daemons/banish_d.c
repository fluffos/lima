...
void create()
{
   ...
   if (sizeof(bad_names) != 0 && stringp(bad_names[0]))
   {
      for (int i = sizeof(bad_names); i--;)
         bad_names[i] = new (class banish_data, item : bad_names[i], reason : "unknown; check the log");
      for (int i = sizeof(bad_sites); i--;)
         bad_sites[i] = new (class banish_data, item : bad_sites[i], reason : "unknown; check the log");
      save_me();
   }
}
...
int check_site(string *check)
{
   ...
   if (!check)
      check = ({query_ip_number(previous_object()), lower_case(query_ip_name(previous_object()))});

   // return non-zero if banished
   return sizeof(filter(bad_sites, ( : sizeof(regexp($(check), translate(((class banish_data)$1)->item))) :)));
}
