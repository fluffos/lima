...
void setup(string guildname)
{
   ...
   set_long("The guildmaster is a large, muscle-bound killing machine. It would be wise to avoid pissing him off. Ask "
            "him to \"join the guild\" or to \"leave the guild\".");
   set_max_health(10);
...
void receive_outside_msg(string str)
{
   ...
   if (search(str, "leave +(|the +)guild") != -1)
   {
      ...
      {
         call_out((
                      : this_body()->simple_action("The Guildmaster says to $n, \"You cannot leave us, my friend.\"")
                      :),
                  0);
      }
   }
