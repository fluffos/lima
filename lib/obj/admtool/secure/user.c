...
private
nomask void nuke_user(string userid, int skip_save)
{
   object o;
   mixed err;
   mapping bodies;
   ...
   {
      // A bit ugly, but does the job. We'd rather store user info in user files than create a daemon to track all
      // these.
      bodies = restore_variable(
          filter_array(explode(read_file(LINK_PATH(userid) + __SAVE_EXTENSION__), "\n"), (
                                                                                             : $1[0..5] == "bodies"
                                                                                             :))[0][7..]);
   }
...
private
nomask void confirm_nuking(string name, string str)
{
   str = lower_case(str);
   if (str != "y" && str != "yes")
   {
...
private
nomask void receive_name_for_nuking(string name)
{
...
private
nomask void receive_name_for_wiz(string name)
{
   object ob;
   string err;
...
private
nomask void enter_new_password(string name, string newpass)
{
   string userfile = LINK_PATH(name) + ".o";
   string *lines;
   ...
   lines = explode(read_file(userfile), "\n");
   for (int i = 0; i < sizeof(lines); i++)
   {
      if (strlen(lines[i]) > 8 && lines[i][0..7] == "password")
      {
         lines[i] = "password \"" + crypt(newpass, 0) + "\"";
      }
   }
   write_file(userfile, implode(lines, "\n"), 1);
   printf("New password set for " + name + ".\n");
}

private
nomask void receive_name_for_reset(string name)
{
   object ob;
   string err;
   ...
   if (ob)
   {
      printf("** Error: '%s' is currently logged on.\n", name);
      return;
   }

   printf("Enter new password: ");
   modal_simple(( : enter_new_password, name:));
}

private
nomask void receive_name_for_dewiz(string name)
{
   object ob;
   string err;
...
private
nomask void receive_position_for_wiz(string name, string position)
{
...
private
nomask void confirm_purge(mixed *times, string str)
{
   str = lower_case(str);
   if (str != "y" && str != "yes")
   {
...
private
nomask void receive_days_for_purge(string days)
{
   int limit = time() - (to_int(days) * SECS_PER_DAY);
   mixed *times = filter(LAST_LOGIN_D->query_times(), ( : $1[0] <= $(limit) :));
...
nomask class command_info *module_commands()
{
   return ({
       ...
       new (class command_info, key
            : "d", proto
            : "[name]", args
            : ({"Who should be de-wizzed? "}), action
            : (
       ...
       new (class command_info, key
            : "p", proto
            : "[name] [position]", args
            : ({"Set position for who? ", "Set %s's postion to what? "}), action
            : (
                  : receive_position_for_wiz:),
              desc
            : "give a wizard a position"),
#endif
       new (class command_info, key
            : "r", proto
            : "[name]", args
            : ({"Reset password of which offline user? "}), action
            : (
       ...
       new (class command_info, key
            : "P", proto
            : "[days]", args
            : ({"How many days since their last login? "}), action
            : (
                  : receive_days_for_purge:),
              desc
            : "Purge users not logged in within N days"),
   });
}
