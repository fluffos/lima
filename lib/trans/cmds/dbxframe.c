...
void main(string str)
{
   ...
   outf("------%s:%i - %s(%s)\n", frame["program"], frame["line"], frame["function"], print_vars(frame["arguments"]));
   outf("locals: %s\n", print_vars(frame["locals"]));
   outf("----------------------------------------------------------------\n");

   before = (frame["line"] > 5 ? 5 : frame["line"] - 1);
   outf("%s=>%s%s", read_file("/" + frame["program"], frame["line"] - before, before),
        read_file("/" + frame["program"], frame["line"], 1), read_file("/" + frame["program"], frame["line"] + 1, 5));
