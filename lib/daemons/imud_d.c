...
private
string *router_list = ({({ROUTER, ROUTER_ADDRESS})});
...
void rcv_startup_reply(string orig_mud, string orig_user, string targ_user, mixed *message);
void rcv_error(string orig_mud, string orig_user, string targ_user, mixed *message);
void rcv_oob_req(string orig_mud, string orig_user, string targ_user, mixed *message);

nosave private mapping dispatch = (["tell":(
                                               : rcv_tell:),
                                 "emoteto":(
                                               : rcv_emoteto:),
...
private
nomask void send_message(string type, string target_mud, string target_user, mixed *message)
{
   string orig_user;
   ...
   if (catch (router_socket->send(({type, 5, mud_name(), orig_user, target_mud, target_user}) + message)))
   {
...
nomask void handle_router_read(object socket, mixed *message)
{
   ...
   if (!dispatch[message[0]])
   {
      ...
      send_message("error", message[2], message[3],
                   ({"unk-type", sprintf("type '%s' is unrecognized", message[0]), message}));
      return;
   }
   evaluate(dispatch[message[0]], message[2], message[3], message[5], message[6..]);
}
...
nomask void reconnect()
{
   ...
   err = catch (router_socket = unguarded(1, (
                                                 : clone_object, SOCKET, SKT_STYLE_CONNECT_M, router_list[0][1],
                                                   (
                                                       : handle_router_read:),
                                                   (
                                                       : handle_router_close:)
...
protected
nomask void log_error_rcv(string mudname, mixed *message)
{
   LOG_D->log(LOG_I3_ERROR, sprintf("(<- %s) %s: %s\n%O\n", mudname, message[0], message[1], message[2]));
...
protected
nomask void log_error_snd(string mudname, mixed *message)
{
   LOG_D->log(LOG_I3_ERROR, sprintf("(-> %s) %s: %s\n%O\n", mudname, message[0], message[1], message[2]));
}

private
nomask void rcv_startup_reply(string orig_mud, string orig_user, string targ_user, mixed *message)
{
   /*
   ...
   */
   ...
   if (message[0][0][0] != router_list[0][0] || message[0][0][1] != router_list[0][1])
   {
...
private
nomask void rcv_error(string orig_mud, string orig_user, string targ_user, mixed *message)
{
   object ob;
...
private
nomask void rcv_oob_req(string orig_mud, string orig_user, string targ_user, mixed *message)
{
   /*
...
string stat_me()
{
   ...
   result += m_reconnect::stat_me() + oob::stat_me();
