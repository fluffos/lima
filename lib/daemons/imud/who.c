...
protected
nomask void rcv_who_req(string orig_mud, string orig_user, string targ_user, mixed *message)
{
   mixed *who_data;
...
protected
nomask void rcv_who_reply(string orig_mud, string orig_user, string targ_user, mixed *message)
{
   object p;
   ...
   if (!p)
   {
      return_error(orig_mud, orig_user, "unk-user", sprintf("Returned who-reply to unknown user '%s'", targ_user));
   }
   else
   {
      string s;
      ...
      s += implode(map_array(message[0], (
                                             : sprintf("%-15s%-10s%s", $1[0], convert_time(to_int($1[1]), 3), $1[2])
                                             :)),
                   "\n");
      s += "\n" + repeat_string("-", 78) + "\n";
      tell(p, s);
