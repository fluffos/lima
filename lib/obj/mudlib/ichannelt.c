   ...
   if (stringp(who))
   {
      ...
      if (mudname)
      {
	    ...
	    if ( this_body() ) move( this_body(), "in" );
         ...
         set_id("wibblewibble" + lower_case(user_name) + " " + replace_string(lower_case(used_mudname), ".", " "));
...
string query_name()
{
   return capitalize(user_name);
}

string short()
{
   return (subj_obj & 1 ? "$N" : (subj_obj & 2 ? "$O" : query_name() + (mudname ? "@" + mudname : "")));
}
string a_short()
{
   return short();
