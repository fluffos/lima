...
private
nomask void show_group_members(string group)
{
   string *members;
...
private
nomask void create_group(string group)
{
   group = lower_case(group);
...
private
nomask void delete_group(string group)
{
   group = lower_case(group);
...
private
nomask void show_users_groups(string user)
{
   mapping groups;

   user = lower_case(user);
   groups = filter(GROUP_D->get_group_data(), ( : member_array($(user), $2) != -1 :));

   if (!sizeof(groups))
      printf("%s doesn't belong to any groups.\n", user);
   else
      printf("%s belongs to: %s.\n", user, implode(sort_array(keys(groups), 1), ", "));
}

private
nomask void add_user_to_group(string user, string group)
{
   user = lower_case(user);
   group = lower_case(group);
...
private
nomask void remove_user_from_group(string user, string group)
{
   user = lower_case(user);
   group = lower_case(group);
...
nomask class command_info *module_commands()
{
   return ({new (class command_info, key
            ...
            new (class command_info, key
                 : "u", proto
                 : "[user]", desc
                 : "show user's groups ", args
                 : ({"Show groups for which user? "}), action
                 : (
                     : show_users_groups:)),
            new (class command_info, key
                 : "a", proto
                 : "[user] [group]", desc
                 : "add user to a group", who
                 : "[Mudlib:]", args
                 : ({"Add which user? ", "Add '%s' to which group? "}), action
                 : (
                     : add_user_to_group:)),
            new (class command_info, key
                 : "r", proto
                 : "[user] [group]", desc
                 : "remove a user from a group", who
                 : "[Mudlib:]", args
                 : ({"Remove which user? ", "Remove '%s' from which group? "}), action
                 : (
