...
private
nomask void receive_topic(string arg);
private
nomask void display_choice(string topic);
...
private
nomask string last_component(string fname)
{
   return (explode(fname, "/") - ({""}))[ < 1];
}

private
nomask void parse_directory(string fname)
{
...
private
nomask string format_choice(string choice)
{
...
private
nomask void lookup_topic(string topic)
{
   string *files;
...
private
nomask void receive_more(mixed arg)
{
   if (arg == -1)
      destruct(this_object());
   if (arg)
...
private
nomask void receive_topic(mixed arg)
{
   if (arg == -1)
      destruct(this_object());
...
private
nomask void display_topic_columns(string *files, string header)
{
   int i;
   int n = sizeof(files);
