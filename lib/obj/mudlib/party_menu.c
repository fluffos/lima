...
void start_menu()
{
   ...
   add_menu_item(toplevel, new_menu_item("Who is in " + party_name + "?",
                                         (: who_current :), "w"));
   ...
   add_menu_item(toplevel, new_menu_item("List offline party memebers.",
                                         (: who_offline :), "o"));

   if(PARTY_D->query_owner(party_name) == this_body()->query_name())
      add_menu_item(toplevel, new_menu_item("Party maintenance",
                                            (: party_maint :), "m"));

   add_menu_item(toplevel, new_menu_item("QUIT " + party_name + " permanently.",
                                         (: quit_party :), "X"));
...
void confirm_create(string owner, string party_name, string response)
{
   switch(response)
   {
      ...
      case "y":
      case "yes":
         modal_simple((: enter_password, owner, party_name, 0 :),
                      "   Enter password: ", 1);
         return;
      default:
         modal_simple((: confirm_create, owner, party_name :),
...
private void confirm_password(string owner,
                              string party_name, string password,
                              int failures, string response)
{
   if(response != password)
   {
      ...
      modal_simple((: enter_password, owner, party_name, failures :),
                   "   Enter password: ", 1);
      return;
   }
                                    ...
                                    string response)
{
   ...
   {
      modal_simple((: confirm_password, owner, party_name,
                      response,  failures :),
                   "\n   Confirm password: ", 1);
      return;
   }
...
private varargs void enter_party_password(string owner,
                                         string party_name,
                                         int failures,
                                         string response)
{
   ...
   if(!PARTY_D->add_member(owner, party_name, response))
   {
      ...
      modal_simple((: enter_party_password, owner, party_name, ++failures :),
...
private void enter_party_name(string owner, string response)
{
   ...
   {
      printf("%s exists.\n", response);
      modal_simple((: enter_party_password, owner, response, 0 :),
                   "   Enter password: ", 1);
   }
   else
   {
...
private void confirm_join_party(string owner, string response)
{
...
void join_party()
{
   ...
   if(!PARTY_D->locate_user(owner))
   {
      ...
      modal_simple((: confirm_join_party, owner :),
