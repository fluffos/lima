...
*/
protected
nomask void print_mod_info(string channel_name)
{
   ...
   {
      if (!ci.requestors || !sizeof(ci.requestors))
...
protected
nomask void moderation_signoff(string channel_name)
{
   class channel_info ci = query_channel_info(channel_name);
   ...
   {
      ci.speaker = 0;
      deliver_notice(channel_name, sprintf("%s is no longer speaking", this_body()->query_name()));
   }
}

protected
nomask int cmd_moderation(string channel_name, string arg)
{
   class channel_info ci = query_channel_info(channel_name);
   ...
   if (arg == "/raise")
   {
      ...
      else if (member_array(tb, ci.requestors) == -1)
      {
         ...
         ci.requestors += ({tb});
         ci->moderator->channel_rcv_string(
             channel_name, sprintf("[%s] (%s raises a hand to speak)\n", user_channel_name, sender_name));
      }
      else
      {
   ...
   else if (arg[0..4] == "/call")
   {
      ...
      else if (arg == "")
      {
         ...
         else
         {
            ...
            deliver_notice(channel_name, sprintf("%s will now speak", ci->speaker->query_name()));
         }
      }
      else
      {
         ...
         spkr = filter_array(ci.requestors, ( : $(arg) == lower_case($1->query_name()) :));
         if (sizeof(spkr) == 0)
         {
   ...
   {
      if (adminp(this_user()) || GROUP_D->member_group(this_user()->query_userid(), "moderators"))
      {
         ci.moderator = tb;
         if (!ci.requestors)
            ci.requestors = ({});
         deliver_notice(channel_name, sprintf("%s is now moderating", sender_name));
      }
      else
      {
