...
private
nosave string *xaliases = ({});
private
nosave mapping alias_save = ([]);
private
nosave string *xalias_save = ({});
...
private
nomask void internal_remove_alias(string alias_name)
{
...
private
string *prep_aliases_for_save(string state)
{
   string a;
...
varargs protected void cmd_alias(mixed argv, string *implode_info)
{
   ...
   default:
      ...
      {
         if (implode_info)
            add_alias(argv[1][1..], implode_by_arr(argv[2..], implode_info[2..])[1..], 0, 1);
         else
            add_alias(argv[1][1..], implode(argv[2..], " "), 0, 1);
         printf("Xverb alias '%s' set to: %s.\n", argv[1][1..], ((class alias)aliases[argv[1][1..]])->template);
         return;
      }
      if (implode_info)
      ...
      printf("Alias '%s' set to: %s.\n", argv[1], ((class alias)aliases[argv[1]])->template);
   }
}
