...
void setup()
{
   ...
   set_light(0);
   set_exits((["south":"Low_Crawl", "east":river, "west":river, "southeast":"Small_Dock", "north":"North_Cave", ]));
   set_hidden_exits("east", "west");
   set_default_error("You can't find an exit in that direction.\n");

   add_item("stalagmite", "stalactite", "stalagmites", "stalagmite", "columns", "column",
            "There is nothing special about the stalagmites and stalactites.");

   add_item("cavern", "cave", "It's big.");

   add_item("river", (["swim":"Swiming in the river accomplishes nothing useful.",
