...
void main(string arg)
{
  ...
  write(e_info + green_bar(xp - body->query_xp_for_level(body->query_level()), body->query_next_xp() - body->query_xp_for_level(body->query_level()), width) + "\n");
  ...
  write(sprintf("" +
                    FC + "Strength     %-4d%s  Dexterity %-4d%s  Intelligence %-4d%s\n" +
                    FC + "Constitution %-4d%s  Charisma  %-4d%s\n",
                "Stats",
                body->query_str(), (pretty_bonus(body->query_str() - body->query_str_pure())),
                body->query_dex(), (pretty_bonus(body->query_dex() - body->query_dex_pure())),
  ...
  write(simple_divider());
  write(sprintf(FC + "Acid Rain: %s  XP Buff: %s\n",
                "Other", (body->call_hooks("acid_protection", HOOK_SUM) != 0 ? "%^GREEN%^Protected%^RESET%^" : "No protection"), 
