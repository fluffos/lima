...
private
nosave object shell_ob; /* which shell ob we're editing aliases for */
private
nosave string *display_menu_options;
private
nosave class alias new_alias_in_progress;
private
nosave string new_alias_name;
private
nosave int num_defaults;
private
nosave int default_index;
...
private
void process_display_alias_menu_choice(string input)
{
   int index;
...
void get_xverb_status(string s)
{
   ...
   {
   case '?':
      write("Do you want to be able to omit the space between the alias and the "
...
void new_template(string s)
{
   ...
   if (s == "?")
   {
      write("An alias definition can change each time you use the alias:\n"
...
private
void rcv_new_alias_name(string s)
{
   if (strsrch(s, " ") != -1)
   {
...
private
void cmd_adjust_alias(string s)
{
   if (strsrch(s, " ") != -1)
   {
      write("Aliases may not have spaces.  Try again.\n");
...
private
varargs void process_remove_alias(mixed s, int not_using_menu)
{
...
private
void input_from_main_menu(string arg)
{
   int cmd;
