...
private
nomask void add_alias(string name, string expansion, int dev, string xinfo)
{
   int xverb;
...
private
nomask void handle_l(string cmd)
{
   string a;
   mixed alias_info;
   string *xaliases;
...
private
nomask void handle_a(string cmd, string name, string expansion)
{
   int type;
   if (cmd == "a")
      type = 0;
...
private
nomask void handle_r(string cmd, string input)
{
   mixed alias_info;
   ...
   return ({new (class command_info, key
            ...
            new (class command_info, key
                 ...
                 : (
                       ...
                       :),
                   desc
                 : "list global wizard aliases"),
            new (class command_info, key
                 : "a", proto
                 : "[alias [expansion]]", args
                 : ({"Alias: ", "Expansion: "}), action
                 : (
                       : handle_a, "a"
                       :),
                   desc
                 : "add a global player alias"),
            new (class command_info, key
                 : "A", action
                 : (
                       : handle_a, "A"
                       :),
                   args
                 : ({"Alias: ", "Expansion: "}), proto
                 : "[alias [expansion]]", desc
                 : "add a global wizard alias"),
            new (class command_info, key
                 : "r", proto
                 : "[alias]", args
                 : ({"Alias: "}), action
