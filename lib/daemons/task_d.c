...
nomask private int check_completed(mixed *task_list)
{
  ...
  foreach (mixed *task in task_list)
...
{
  mixed *parent_id = explode(task_id, ".")[ < 2.. < 2];
...
string add_task(string parent_id, string title, string description, string who)
{
  ...
  {
    mixed *task = find_task(parent_id);
...
*remove_task(string task_id)
{
  ...
  {
    mixed *parent = find_task(parent_id);
    mixed *p_sub = parent[TASK_SUB];
