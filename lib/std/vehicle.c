...
private
nosave int notify_all = 1;
private
nosave string entrancemsg;
private
nosave string exitmsg;
nosave function set_driving_hook;
nosave function change_driving_hook;

protected
void set_move_notification(int a)
{
   notify_all = a;
}

protected
void set_arrival_msg(string s)
{
   add_msg("enter", s);
}

protected
void set_departure_msg(string s)
{
   add_msg("leave", s);
}
...
string get_riders_as_string()
{
   ...
   return implode(map(riders[0.. < 2], ( : $1->short() :)), ", ") + " and " + riders[ < 1]->short();
}
