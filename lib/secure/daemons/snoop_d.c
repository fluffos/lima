...
private
object get_dummy_object(object snoopee)
{
...
// some new guy...
private
int detect_cycle_recursive(object snooper)
{
   object *snoopees;
   int *cycles;
...
private
int detect_cycle(object snoopee)
{
   cycle_info = ([]);
   admins = ({});
...
int unsnoop(object snooper, object snoopee)
{
   object *snoopees;
   object target;
   if (origin() != "local" && previous_object() != find_object(UNSNOOP_CMD))
      return -2;
   snoopees = snoops[snooper];
   ...
   admins->receive_private_msg(capitalize(snooper->query_userid()) + " is no longer snooping you.\n");
   return 0;
}
...
int snoop(object snooper, object snoopee)
{
   ...
   admins->receive_private_msg(capitalize(snooper->query_userid()) + " is now snooping you!\n");
   return 0;
}
