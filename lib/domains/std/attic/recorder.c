...
string get_item_desc(string str)
{
   ...
   case "record":
   case "record button":
      return sprintf("It's red and says, 'record'.%s", state == RECORDING ? "  It's currently pressed down." : "");
   case "play":
   case "play button":
...
int press(string name)
{
   ...
   case "play button":
      ...
      state = INACTIVE;
      this_body()->simple_action("$N $vpush the play button on $p recorder, and a transcript magically appears before "
                                 "$n, in lieu of an actual playback.");
