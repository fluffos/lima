...
nomask void do_emoteto(string mudname, string username, string message)
{
   ...
   send_to_user("emoteto", canon_mudname(mudname), lower_case(username), ({this_body()->query_name(), message}));
}

protected
nomask void rcv_emoteto(string orig_mud, string orig_user, string targ_user, mixed *message)
{
   object p;
