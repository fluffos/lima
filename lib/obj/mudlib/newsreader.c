...
private
nosave string current_group;
private
nosave int current_id;
private
nosave int current_thread;
private
nosave int queue_position;
private
nosave int *message_queue = ({});
private
nosave int *all_messages = ({});
private
nosave mapping active_messages = ([]);
private
nosave mapping unread_cache = ([]);
...
private
nomask void display_groups_with_new(string arg);
...
varargs private string get_unread_ids(string group, int update)
{
   ...
   {
      string unread =
          set_difference(set_add_range("", 1, NEWS_D->get_group_last_id(group)), SAVE_OB->get_news_id_read(group));
...
private
int get_lowest_unread_id(string group)
{
   int i;
...
private
int get_lowest_unread_thread_id(string group)
{
   return NEWS_D->get_thread_id(group, get_lowest_unread_id(group));
}

private
void receive_search(int flag, string str)
{
   mixed *results;
   if (!sizeof(str))
   {
...
private
int count_active_messages(string group)
{
   return sizeof(active_messages[group]);
}

private
int count_unread_messages(string group)
{
...
varargs private void update_queues()
{
   ...
   all_messages = active_messages[current_group];
   message_queue = clean_array(message_queue + filter(all_messages,
                                                      (
                                                          : member_set:),
                                                      get_unread_ids(current_group, 1)));
   if (queue_position != -1)
...
private
nomask string format_group_line(string group)
{
   int last_id;
   int all;
   current_group = group;
   ...
   return sprintf("%s  %-40s (%d %s, %d new)%%^RESET%%^",
                  SAVE_OB->check_subscribed(group) ? "%^NEWS_GROUP_SUBSCRIBED%^" : "%^NEWS_GROUP_UNSUBSCRIBED%^", group,
                  all, all == 1 ? "message" : "messages", count_unread_messages(group));
}
...
nomask void rcv_group_answer(string *groups, string input)
{
   ...
   case "q":
      return;
   default:
      modal_simple(( : rcv_group_answer, groups:), sprintf("Subscribe to %s [ynq]? ", groups[0]));
      return;
   }
   groups -= ({groups[0]});
   if (sizeof(groups))
...
private
nomask string grp_cmd_prompt()
{
   int unread_no = count_unread_messages(current_group);
   return sprintf("(%s:%d) %d %s unread of %d [q?lLmgsSputncU] > ", current_group,
                  sizeof(message_queue) > 1 ? current_id + 1 : 0, unread_no, unread_no == 1 ? "msg" : "msgs",
                  count_active_messages(current_group));
}

private
nomask string msg_cmd_prompt()
{
   return sprintf("(%s:%d:#%d) %d unread [q?lLmghsSprRfFncUMDut#g] > ", current_group, queue_position + 1, current_id,
                  count_unread_messages(current_group));
}

private
nomask void switch_to_top()
{
   modal_push(( : receive_top_cmd:), TOP_PROMPT);
}

private
nomask void switch_to_group()
{
   modal_func(( : receive_grp_cmd:), ( : grp_cmd_prompt:));
}

private
nomask void switch_to_message()
{
   modal_func(( : receive_msg_cmd:), ( : msg_cmd_prompt:));
}

private
nosave mixed group_selection_menu_items;
private
nomask void menu_select_newsgroup(string num)
{
   int index;
   if (num == "q")
   {
   ...
   if (!sscanf(num, "%d", index) || index < 1 || index > sizeof(group_selection_menu_items))
   {
...
private
nomask void menu_select_change_header(string header)
{
   NEWS_D->change_header(current_group, current_id, header);
}

private
nomask void menu_select_movegroup(string num)
{
   string to_group;
   int index;
   if (num == "q")
...
private
nomask void read_group(string group)
{
   string *matches;
   int i;
...
private
nomask int test_for_new(string group)
{
   if (count_unread_messages(group) > 0)
      return 1;
   return 0;
}
...
nomask void display_groups_with_new(string arg)
{
   ...
   */
   ...
   {
      string *list;
      list = ({"Groups with new messages:", ""}) + map_array(groups, ( : format_group_line:));
...
varargs private nomask string format_message_line(int short_fmt, int id, int all)
{
   class news_msg msg;
   int i = 0;
   if (current_id > -1)
...
private
nomask void display_messages(int display_all)
{
   int *ids;
   string *lines;
   if (!display_all)
   ...
   lines += map_array(ids, ( : format_message_line(0, $1, ($(display_all) ? 1 : 0)) :));
   more(lines);
}
...
private
nomask void receive_post_text(string subject, string *text)
{
   int id;
   if (!text)
   {
...
private
nomask void receive_post_subject(mixed subject)
{
   if (subject == -1)
      destruct(this_object());
...
private
nomask void reply_with_message()
{
   class news_msg msg = ((class news_msg)NEWS_D->get_message(current_group, current_id));
   string *lines;
   if (!msg)
...
nomask void receive_followup_text(string *text)
{
   ...
   ** message to read.
   */
   id = NEWS_D->followup(current_group, current_id, implode(text, "\n") + "\n");
   write("Posted:  " + format_message_line(1, id) + "\n");
}
...
private
nomask void followup_with_message()
{
   class news_msg msg = ((class news_msg)NEWS_D->get_message(current_group, current_id));
   string *lines;
   if (!msg.body)
...
private
nomask void read_message(string group, int id)
{
   class news_msg msg;
   string post;
   msg = NEWS_D->get_message(current_group, id);
...
private
nomask void toggle_subscription(string group)
{
...
private
nomask void global_commands(string cmd)
{
   cmd = trim(cmd);
   if (cmd == "q")
   {
   ...
   {
      if (sizeof(filter_array(SAVE_OB->subscribed_groups(), ( : test_for_new:))))
      {
         modal_pop();
         begin_reading();
         return;
   ...
   else if (cmd[0] == 'g')
   {
      ...
      {
         modal_func(( : receive_group:), "Which group (Or enter for a menu)? ");
      }
   }
   else
   {
...
private
nomask void group_commands(string cmd)
{
   int id;
   if (cmd == "l" || cmd == "L")
   {
...
private
nomask void receive_remove_verify(string str)
{
   string group = current_group;
   if (str[0] != 'y' && str[0] != 'Y')
   {
...
private
nomask void get_move_group(string str)
{
   string *matches;
   int i;
   ...
   case 0:
      ...
      return;
   case 1:
      NEWS_D->move_post(current_group, current_id, matches[0]);
      break;
   default:
      write("\n"
            "Select group by number:\n"
...
private
nomask void receive_move_verify(string str)
{
   if (str[0] != 'y' && str[0] != 'Y')
   {
      write("Move aborted.\n");
...
nomask void remove_message()
{
   ...
   printf("Deleting: %s\nAre you sure? [yn] > ", format_message_line(1, current_id, 1));
...
nomask void receive_grp_cmd(mixed cmd)
{
   ...
   if (cmd == "?")
   {
      write("\n"
            ...
            "current newsgroup you are reading (" +
            current_group +
