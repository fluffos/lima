...
private
nomask void receive_privilege_for_show(string priv)
{
   int n;
   mapping privmap;
...
private
nomask void receive_privilege_for_define(string priv)
{
...
private
nomask void receive_privilege_for_undefine(string priv)
{
   /* checks for admin/owner... */
...
private
nomask void receive_privilege_for_add(string user, string priv)
{
   /* checks for admin/owner... */
...
private
nomask void receive_privilege_for_remove(string user, string priv)
{
   /* checks for admin/owner... */
...
private
nomask void remap_tree(mapping result, mapping tree, string path)
{
   string subdir;
   mixed *data;
...
private
nomask void list_dir_privs(string arg)
{
   mapping r_tree;
   mapping w_tree;
   string dir;
   ...
   output = implode(({""}) + sort_array(keys(w_tree), 1), ( : $1 + $2 + "   " + $(w_tree)[$2] + "\n" :)) + "\n";
   more(output);
}

private
nomask void perform_set_dir_priv(int rw, string dir, mixed priv)
{
...
private
nomask void perform_clear_dir_priv(string dir)
{
   dir = evaluate_path(dir);
...
nomask class command_info *module_commands()
{
   return ({
       ...
       new (class command_info, key
            : "s", proto
            : "[priv]", desc
            : "show sub-privileges", who
            : "[admin/owner]", args
            : ({"Show information for which privilege? "}), action
            : (
                : receive_privilege_for_show:)),
       new (class command_info, key
            : "n", proto
            : "[priv]", desc
            : "define new privilege", who
            : "[admin/owner]", args
            : ({"Define which privilege? "}), action
            : (
                : receive_privilege_for_define:)),
       new (class command_info, key
            : "u", proto
            : "[priv]", desc
            : "undefine privilege", who
            : "[admin/owner]", args
       ...
       new (class command_info, key
            : "a", proto
            : "[priv [priv]]", desc
            : "add a privilege to another", who
            : "[admin/owner]", args
            : ({"Adding which user (to a priv) ? ", "Add '%s' to which privilege? "}), action
            : (
                : receive_privilege_for_add:)),
       new (class command_info, key
            : "d", proto
            : "[priv [priv]]", desc
            : "delete a privilege from another", who
            : "[admin/owner]", args
       ...
       new (class command_info, key
            : "l", proto
            : "[dir]", desc
            : "list directory protections", args
            : ({"Which directory? "}), action
            : (
                : list_dir_privs:)),
       new (class command_info, key
            : "w", proto
            : "[dir priv]", desc
            : "set a directory's write protection", who
            : "[parent priv]", args
            : ({"Set which directory's write privilege? ", "Set '%s' to what privilege? "}), action
            : (
       ...
       new (class command_info, key
            : "r", proto
            : "[dir priv]", desc
            : "set a directory's read protection", who
            : "[admin]", args
            : ({"Set which directory's read privilege? ", "Set '%s' to what privilege? "}), action
            : (
       ...
       new (class command_info, key
            : "c", proto
            : "[dir]", desc
            : "clear a directory's protection", who
            : "[parent priv]", args
            : ({"Clear which directory's privileges? "}), action
            : (
