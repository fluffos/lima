...
mapping removed = ([]);
private
nosave mapping recent_changes = ([]);
private
nosave mapping threading_data = ([]);
// No info on a group means never archive.
private
mapping archive_info = ([]);
...
private
nosave mapping additional_restrictions = ([]);

private
mapping restrictions = ([]);

private
mapping mail_forward = ([]);
...
nomask void create()
{
   ...
   {
      string *groups =
          ({BUG_NEWSGROUP, TYPO_NEWSGROUP, IDEA_NEWSGROUP, TODO_NEWSGROUP, QUESTION_NEWSGROUP, FEEDBACK_NEWSGROUP});
      ...
      foreach (string key, mixed value in recent_changes)
      {
         ...
         else
         {
            ...
            foreach (string key2 in keys(value))
            {
               ...
               if (classp(value[key2]) && ((class news_msg)value[key2])->body == 0)
                  removed[key] += ({key2});
            }
         }
      }
...
private
nomask int get_new_id(string group)
{
...
nomask string *query_mail_forward(string group)
{
   if (!check_privilege("Mudlib:daemons"))
      return 0;
   if (member_array(group, keys(mail_forward)) == -1)
      return ({});
...
private
nomask void do_mail_forward(string group, class news_msg msg)
{
   string body;
   body = sprintf("Posted by %s to %s on %s:\n\n%s\n", msg.poster, group, MUD_NAME,
...
nomask int is_write_restricted(string group)
{
   ...
   {
      if (member_array(lower_case(restrict[0]), SECURE_D->query_domains(whoami)) > -1 &&
...
nomask int is_read_restricted(string group)
{
   ...
   if ('A' <= closest[0] && closest[0] <= 'Z')
   {
      if (member_array(lower_case(restrict[1]), SECURE_D->query_domains(whoami)) > -1 &&
          member_array(whoami, SECURE_D->query_domain_lords(lower_case(restrict[1]))) > -1)
         return 0;
      else
         return 1;
   }
...
nomask int *get_thread(string group, int thread)
{
   ...
   return filter_array(keys(data[group]),
                       (
                           : $1 != "next_id" && ((class news_msg)data[$(group)][$1])->thread_id == $(thread)
                           :));
}
...
private
nomask void archive_post(string group, int id)
{
...
private
nomask int sort_messages_by_thread(int first, int second, string group)
{
...
nomask void update_threads(string group)
{
   threading_data[group] = sort_array(get_messages(group, 1), ( : sort_messages_by_thread:), group);
}
