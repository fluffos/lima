...
string main(string pattern)
{
   ...
   if (sizeof(mudlist) != 1)
   {
      ...
      foreach (mudname in sort_array(keys(mudlist), 1))
      {
         ...
         ret += sprintf("<a href=mudinfo.c?=%s$>%s</a><br>", replace_string(mudname, " ", "+"), mudname);
      }
      return ret + "</em></font>";
   }
   foreach (mudname, data in mudlist) // there's only 1
   {
      ret += sprintf("<TABLE BORDER=5><caption><h2><a href="
                     ...
                     "</TABLE>",
                     data[1], data[2], mudname, data[8], data[5], get_mudlib(data[6]), data[7],
                     data[0] == -1 ? "Connected" : "Not connected", data[9], mail_link(data[10]));
   }
   return ret;
}
