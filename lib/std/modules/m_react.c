   ...
   while (pi->cur < sizeof(pi.lines))
   {
      ...
      if (line[0] == '@')
      {
         ...
         case "function":
            ...
            program += ({({"function", line}) + parse_file(pi, ({"@endfunction"}))});
            break;
         case "move":
         case "callscript":
         ...
         case "doevery": {
            ...
            program += ({({"doevery", first, second}) + parse_file(pi, ({"@enddoevery"}))});
            break;
         }
         case "oneof":
         ...
         case "delay":
            tmp = to_int(line || "2");
            if (tmp <= 0)
               error("Bad interval " + line + ".\n");
            program += ({({"delay", tmp}) + parse_file(pi, term)});
            return program;
            break;
         default:
...
void compile_script(string fname)
{
   ...
   pi = new (class parse_info, lines : explode(read_file(fname), "\n"), vars : ({}));
