...
private
string open_msg = "$N $vopen a $o.";
private
string close_msg = "$N $vclose a $o.";
private
string open_desc;
private
string closed_desc;
...
int query_closed()
{
   return !test_flag(F_OPEN); /* closed; */
}
...
string query_closed_desc()
{
   return closed_desc;
}

string query_open_desc()
{
   return open_desc;
}
...
varargs int open_with(object with)
{
   ...
   if (!ex)
      ex = capitalize(the_short() + " doesn't seem to want to open.\n");
   if (stringp(ex))
   {
      write(ex);
      return 1;
...
mixed do_close()
{
   ...
   if (!tmp)
      tmp = capitalize(the_short()) + " doesn't seem to want to close.\n";
   if (stringp(tmp))
      return tmp;
