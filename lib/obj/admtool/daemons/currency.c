...
nomask void list_currencies()
{
   ...
   {
      printf("%-12s%-12s%12d\n", capitalize(type), capitalize(MONEY_D->query_plural(type)),
             MONEY_D->query_exchange_rate(type));
   }
}

private
nomask void add_currency(string type, string plural)
{
...
private
nomask void set_exchange_rate(string type, string rate_str)
{
...
private
nomask void list_denomination(string type)
{
   ...
   {
      printf("   %-12s%-12s  %12.4f      %12d\n", name, MONEY_D->query_plural(name), MONEY_D->query_factor(name),
             to_int(MONEY_D->query_factor(name) * to_float(MONEY_D->query_exchange_rate(type))));
   }
}

private
nomask void add_denomination2(string type, string name, string plural, string factor_str)
{
...
private
nomask void add_denomination1(string type, string name)
{
   input_two_args("Plural of denomination? ", "Exchange rate factor? ", ( : add_denomination2, type, name:));
}

private
nomask void remove_denomination(string denomination)
{
   string type;
...
nomask class command_info *module_commands()
{
   return ({
       ...
       new (class command_info, key
            : "A", proto
            : "[type] [plural name]", desc
            : "add a currency", who
            : "[Mudlib:daemons]", priv
            : "Mudlib:daemons", args
            : ({"What type? ", "What plural name? "}), action
            : (
                : add_currency:)),
       new (class command_info, key
            : "R", proto
            : "[type]", desc
            : "remove a currency", who
            : "[Mudlib:daemons]", priv
            : "Mudlib:daemons", args
            : ({"What type? "}), action
            : (
       ...
       new (class command_info, key
            : "e", proto
            : "[type] [rate]", desc
            : "set exchange rate", who
            : "[Mudlib:daemons]", priv
            : "Mudlib:daemons", args
            : ({"What type? ", "What rate? "}), action
            : (
                : set_exchange_rate:)),
       new (class command_info, key
            : "d", proto
            : "[type]", desc
            : "list denominations", args
            : ({"What type? "}), action
            : (
                : list_denomination:)),
       new (class command_info, key
            : "a", proto
            : "[type] [denomination]", desc
            : "add a denomination to a currency", priv
            : "Mudlib:daemons", who
            : "[Mudlib:daemons]", args
            : ({"What type? ", "Add which denomination? "}), action
            : (
                : add_denomination1:)),
       new (class command_info, key
            : "", desc
            : "(creates currency type if needed)"),
       new (class command_info, key
            : "r", proto
            : "[denomination]", desc
            : "remove a denomination", priv
            : "Mudlib:daemons", who
            : "[Mudlib:daemons]", args
            : ({"Remove which denomination? "}), action
            : (
                : remove_denomination:)),
   });
}
