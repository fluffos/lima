      ...
      else if (lines[i][0..2] == "//:")
      {
         ...
         else if (sscanf(line, "FUNCTION %s", line) == 1)
         {
            if (func_name(line) != line)
               LOG_D->log(LOG_AUTODOC, "Bad function name: " + fname + " line " + i + ": " + line + "\n");
            ...
            {
               // Strip end '::' if added for RST parsing into a single ':'
               if (strlen(lines[i]) > 5 && lines[i][ < 2..] == "::")
                  lines[i] = lines[i][0.. < 3] + ":";

               // Strip a single leading space if added.
               if (strlen(lines[i]) > 3 && lines[i][2] == ' ')
                  write_file(outfile, lines[i][3..] + "\n");
               else
                  write_file(outfile, lines[i][2..] + "\n");
               i++;
            }
