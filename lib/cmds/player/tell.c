...
void main(string arg)
{
   ...
   if (arg[0] == ':' || arg[0] == ';')
   {
      ...
      if (!soul_ret)
      {
         mystring = sprintf("%%^TELL%%^You emote to %s: %%^RESET%%^%s %s\n",
                            who == this_user() ? "yourself" : capitalize(who->query_userid()),
                            capitalize(this_user()->query_userid()), arg);
   ...
   else
   {
      mystring = sprintf("%%^TELL%%^You tell %s:%%^RESET%%^ %s\n",
                         who == this_user() ? "yourself" : capitalize(who->query_userid()), arg);
      deststring = "%^TELL%^" + capitalize(this_user()->query_userid()) + " tells you: %^RESET%^" + arg + "\n";
   }
