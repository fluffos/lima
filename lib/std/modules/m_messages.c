...
string *query_msg_types()
{
  return clean_array(keys(messages) +
                     keys(MESSAGES_D->query_messages(def_message_type)));
}
...
void inform(object *who, string *msgs, mixed others)
{
  ...
  if (arrayp(others))
    map_array(others - who, (
                                : tell_from_outside($1, $(msgs[ < 1]), MSG_INDENT)
                                :));
  else if (others)
...
varargs void filtered_inform(object *who, string *msgs, mixed others, function filter, mixed extra)
{
  ...
  for (i = 0; i < sizeof(who); i++)
  {
    ...
    if (evaluate(filter, who[i], extra))
      tell(who[i], msgs[i], MSG_INDENT);
  }
  if (arrayp(others))
    map_array(others - who, (
  ...
  else if (others)
  {
    if (arrayp(others))
      ...
      {
        if (evaluate(filter, other, extra))
          tell_from_inside(other, msgs[sizeof(who)], MSG_INDENT, who);
      }
    else if (evaluate(filter, others, extra))
      tell_from_inside(others, msgs[sizeof(who)], MSG_INDENT, who);
  }
}

//: FUNCTION filtered_simple_action
...
varargs void filtered_simple_action(mixed msg, function filter, mixed extra, mixed *obs...)
{
  ...
  if (evaluate(filter, this_object(), extra))
  {
    tell(this_object(), us, MSG_INDENT);
  }
  foreach (object ob in all_inventory(this_object()))
  {
    if (evaluate(filter, ob, extra))
    {
      tell_from_outside(ob, others, MSG_INDENT);
    }
  }

  environment_people = all_inventory(environment(this_object()));
  environment_people -= who;
  foreach (object ob in environment_people)
  {
    if (evaluate(filter, ob, extra))
    {
      tell_from_outside(ob, others, MSG_INDENT);
