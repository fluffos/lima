...
class item init_item(object ob)
{
  ...
  item = new (class item, short
              ...
              : ob->long(),
                ids
              : ob->query_id() + ({ob->short(), ob->plural_short()}),
                value
...
void buy_object(object ob)
{
  ...
  if (this_body()->test_skill("misc/life/haggle", to_int(cost), train_limit))
  {
    ...
    if (increase)
      this_body()->targetted_action(haggle_buy, this_object(), MONEY_D->currency_to_string(increase, currency_type), ob);
    else
...
int sell_object(object ob)
{
  ...
  if (this_body()->test_skill("misc/life/haggle", to_int(cost), train_limit))
  {
    ...
    if (decrease)
      this_body()->targetted_action(haggle_sell, this_object(), MONEY_D->currency_to_string(money, currency_type), ob);
    else
      tell(this_body(), this_object()->query_name() + " smiles at you.\n");
  }
  ...
  this_body()->my_action("$N $vbuy a $o for " + MONEY_D->currency_to_string(cost, currency_type) + ". You give " + MONEY_D->currency_to_string(money[0], currency_type) + " to $o1" + (sizeof(money[1]) ? " and get " + MONEY_D->currency_to_string(money[1], currency_type) + " as change" : "") + ".\n", ob, this_object());
