...
private
string make_l_flag_line(string basename, string fullname)
{
   if (is_dir(fullname))
   ...
   return sprintf("%-10d %20s %s", info[fullname][0], ctime(info[fullname][1])[4..], basename);
}

private
mixed map_ls_arrays(function f, mixed arr1, mixed arr2)
{
   int i, s;
   mixed res;
...
private
string path_join(string s1, string s2)
{
   if (s1[ < 1] == '/')
      return s1 + s2;
   else
...
int do_ls(mixed argv, mapping flags)
{
   ...
   if (!argv[0])
      ...
      argv[0] = decompose(map(argv[0], ( : is_directory($1) ? glob(path_join($1, "*")) : $1:)));
   ...
   foreach (path in sort_array(keys(argv[0]), 1))
   {
      ...
      if (flags["l"])
         outarr = map_ls_arrays(( : make_l_flag_line:), outarr, fullpatharr);
      else if (flags["s"])
         outarr = map_ls_arrays(( : (size($2) + 1024) / 1024 + " " + $1:), outarr, fullpatharr);

      if (!flags["p"] && !uses_ansi)
         outarr = map_ls_arrays((
                                    : is_dir($1) ? $2 + "/"
                                      : info[$1] ? info[$1][2] ? $2 + "*" : $2
                                                 : $2 + "?"
                                    :),
