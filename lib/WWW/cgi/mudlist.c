...
string main(string pattern)
{
   ...
   ret += sprintf("<TABLE BORDER=5><caption><h2>There are currently " + sizeof(matches) + " muds UP out of " +
                  sizeof(mudlist) + " muds on the I3:" + "</h2></caption>" +
   ...
   foreach (string mudname in sort_array(matches, 1))
   {
      ...
      ret += sprintf("<TR><TD><font size=+2><em>" + "<a href=telnet://%s:%d>%s</a></em>" +
                         ...
                         "</TD><TD>%s</TD>"
                         "<TD>%s</TD></TR>",
                     data[1], data[2], mudname, get_mudlib(data[6]), get_driver(data[7]), data[8], data[9],
                     mail_link(data[10]));
   }
   ret += "</TABLE><p><p>";
   return ret;
