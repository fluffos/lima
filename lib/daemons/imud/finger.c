...
protected
nomask void rcv_finger_req(string orig_mud, string orig_user, string targ_user, mixed *message)
{
   class finger info;
...
protected
nomask void rcv_finger_reply(string orig_mud, string orig_user, string targ_user, mixed *message)
{
   object p;
   ...
   if (!p)
   {
      return_error(orig_mud, orig_user, "unk-user", sprintf("Returned finger-reply to unknown user '%s'", targ_user));
   }
   else
   {
      string s;
      ...
      if (sizeof(message) < 9)
         message += allocate(9 - sizeof(message));
      if (message[1] == "")
         message[1] = 0;
      if (message[2] == "")
         message[2] = 0;
