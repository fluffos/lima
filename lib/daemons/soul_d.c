...
int move_emote(string verb, string dest)
{
...
mixed query_emote(string em)
{
   return emotes[em];
}

private
string get_completion(string);
...
mixed *internal_get_soul(string verb, string rule, mixed *args, int add_imud_msg)
{
   ...
   if (add_imud_msg)
      result = ({who, allocate(sizeof(who) + 1), allocate(sizeof(imud_who) + 1)});
   else
   ...
   for (j = 0; j < add_imud_msg + 1; j++)
   {
      ...
      // ### tmp fix
      result[1 + j][ < 1] = compose_message(0, stringp(soul) ? soul : soul[1], w, args...);
   }
...
varargs mixed *get_imud_soul(string verb, string rule, mixed args...)
{
...
string *emote_apropos(string str)
{
...
private
string get_completion(string s)
{
   string *completions;
...
void do_verb_rule(string verb, string rule, mixed args...)
{
   ...
   if (!soul)
      return;
   if (sizeof(soul[0]) == 2 && !immediately_accessible(soul[0][1]))
   {
      soul[1][0] = "*" + soul[1][0];
      soul[1][1] = "*" + soul[1][1];
...
mixed *parse_imud_soul(string str)
{
   ...
   if (sscanf(str, "%s@%s", user, tmp) == 2)
   {
      ...
      for (i = 0; i < j; i++)
      {
         ...
         if (!sizeof(matches))
         {
            write("Vague mud name.  could be: " + implode(previous_matches, ", ") + "\n");
            return;
         }
