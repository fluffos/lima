...
nosave private function remap_name = ( : lower_case(replace_string($1, " ", ".")) :);

protected
nomask int query_mudlist_id()
{
   return mudlist_id;
}
...
protected
nomask void mudlist_reset_entries()
{
...
protected
nomask void rcv_mudlist(string orig_mud, string orig_user, string targ_user, mixed *message)
{
   string mudname;
   mixed *info;
   ...
   mud_names = ([]);
   map_array(keys(mud_info), ( : $(mud_names)[evaluate(remap_name, $1)] = $1:));
}
