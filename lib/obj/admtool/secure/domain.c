...
private
nomask void show_domain_info(string domain)
{
   string *members;
   string *lords;
...
private
nomask void create_domain(int no_domain_dir, string domain)
{
...
private
nomask void delete_domain(string domain)
{
...
private
nomask void show_users_domains(string user)
{
   string *domains;
   ...
   if (!sizeof(domains))
      ...
      printf("Wizard %s belongs to %s.\n", user, implode(sort_array(domains, 1), ", "));
}

private
nomask void add_user_to_domain(int lord, string user, string domain)
{
...
private
nomask void remove_user_from_domain(string user, string domain)
{
   /* this checks for lord privs... */
...
nomask class command_info *module_commands()
{
   return ({
       ...
       new (class command_info, key
            ...
            : "[domain]", who
            : "[admin]", args
            : ({"Name for the new domain? "}), action
            : (
                : create_domain, 0
                :)),
       new (class command_info, key
            : "C", desc
            : "create abstract domain (no dir)", proto
            : "[domain]", who
            : "[admin]", args
            : ({"Name for the new domain? "}), action
            : (
       ...
       new (class command_info, key
            ...
            : "[user]", args
            : ({"Show information for which user? "}), action
            : (
                : show_users_domains:)),
       new (class command_info, key
            : "a", desc
            : "add user to a domain", who
            : "[lord]", proto
            : "[user] [domain]", args
            : ({"Add which user? ", "Add '%s' to which domain? "}), action
            : (
                : add_user_to_domain, 0
                :)),
       new (class command_info, key
            : "A", desc
            : "add user as a lord of the domain", who
            : "[lord]", proto
            : "[user] [domain]", args
            : ({"Add which user as a lord? ", "Add '%s' to which domain? "}), action
            : (
                : add_user_to_domain, 1
                :)),
       new (class command_info, key
            : "r", desc
            : "remove a user from a domain", who
            : "[lord]", proto
            : "[user] [domain]", args
            : ({"Remove which user? ", "Remove '%s' from which domain? "}), action
            : (
