...
private
mapping outgoing_queue = ([]);

// This one maps msgId : mail_msg.
private
mapping internal_queue = ([]);

private
int is_currently_up(string name)
{
   return member_array(IMUD_D->canon_mudname(name), IMUD_D->query_up_muds()) != -1;
}
...
private
void send_warning(int id, string *warning)
{
...
private
void report_errors(int id, string *errorset)
{
   class outgoing_info this_info;
   string recipient;
...
private
void error_ambiguous_or_unknown_mudnames(class mail_msg msg, mapping info)
{
   string *problems = ({});
   ...
   foreach (whatYouCalledIt, whatItCouldHaveBeen in info)
   {
      ...
      {
         problems += ({sprintf("Ambigous mud name, '%s'.  Could have been: %s", whatYouCalledIt,
                               implode(whatItCouldHaveBeen, ", "))});
      }
      else
      {
...
varargs private void handle_ack(mapping ack_list, int flag)
{
   ...
   foreach (id, errorset in ack_list)
   {
      ...
      if (arrayp(errorset) && sizeof(errorset))
      {
         ...
         {
            report_errors(id, ({"The following delivery errors occured:"}) + errorset);
         }
      }
...
private
mapping find_remote_recipients(class mail_msg msg)
{
   mapping remote_info = ([]);
   string name;
...
private
string *get_local_recipients(mixed *mail_packet)
{
   string *mudnames;
...
string *incoming_mail(string mudname, mixed *mail_packet)
{
   ...
   while (i--)
   {
      ...
      {
         errors += ({sprintf("'%s' is not a user on %s.", local_recipients[i], mud_name())});
   ...
   if (sizeof(local_recipients))
   {
      unguarded(1,
                (
                    : MAIL_D->send_mail($(mail_packet)[2] + "@" + $(mudname),
                                        $(mail_packet)[7], // subject
                                        $(mail_packet)[8], // body
                                        targ_map_to_list($(mail_packet)[3], 1), targ_map_to_list($(mail_packet)[4], 1),
