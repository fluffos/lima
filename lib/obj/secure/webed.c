...
string format_editor(string user, string password, string filename, string contents)
{
   ...
   if (contents)
   {
      ...
      if ((ob = load_object(filename)) && sizeof(inh = inherit_list(ob))) {
          ...
          inh = ({ "<OPTION VALUE='" + HTML_encode(inh[0]) + "' SELECTED>" + HTML_encode(inh[0]) }) + map(inh[1..], (:
      "<OPTION VALUE='"+HTML_encode($1)+"'>"+HTML_encode($1) :)); tmp = implode(inh, " ");
...
string display_error(string error, mapping form)
{
   return sprintf("<table border=1><tr><td><font size=+2>%s</font></td></tr>"
                  "</table><p>%s\n",
                  error, format_editor(form["user"], form["password"], form["saveas"], form["contents"]));
}
...
int validate_user(mapping form)
{
   ...
   set_privilege(1);
   info = unguarded(1, ( : USER_D->query_variable($(form["user"]), ({"password"})) :));
   set_privilege(0);
...
string save_text_to_file(mapping form)
{
   ...
   result = unguarded(get_priv(form), ( : write_file($(filename), $(contents), 1) :));

   if (!result)
   {
      return display_error(sprintf("%s: COULD NOT SAVE FILE.\n", filename), form);
   }
   return format_editor(form["user"], form["password"], filename, form["contents"]);
}
