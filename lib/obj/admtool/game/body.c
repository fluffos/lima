...
private
nomask void print_body(string cmd, string bodytype)
{
...
private
nomask void add_body(string cmd, string bodytype)
{
...
private
nomask void finish_add_limb(string bodytype, string limb, int health, string parent, int flags)
{
...
private
nomask void set_limb_flags(string bodytype, string limb, int health, string parent, string flags)
{
   int limbflags;
   if (limbflags = to_int(flags))
   {
      if (limbflags > 31)
      {
         input_one_arg("Limb flags cannot sum to more than 31: ", ( : set_limb_flags, bodytype, limb, health, parent:));
         return;
      }
   }
   else
...
private
nomask void set_limb_parent(string bodytype, string limb, int health, string parent)
{
...
private
nomask void set_limb_health(string bodytype, string limb, string health)
{
   if (!to_int(health))
   {
      input_one_arg("You must enter an integer value: ", ( : set_limb_health, bodytype, limb:));
      return;
   }
   input_one_arg("Parent Limb: ", ( : set_limb_parent, bodytype, limb, to_int(health) :));
}
...
private
nomask void remove_limb(string cmd, string bodytype, string limb)
{
...
private
nomask void remove_body(string cmd, string bodytype)
{
...
nomask class command_info *module_commands()
{
   return ({new (class command_info, key
            ...
            new (class command_info, key
                 : "a", proto
                 : "[bodytype] [limb]", args
                 : ({"Bodytype: ", "Limb: "}), action
                 : (
                       : add_limb, "a"
                       :),
                   desc
                 : "add limb to body type"),
            new (class command_info, key
                 : "r", proto
                 : "[bodytype] [limb]", args
                 : ({"Bodytype: ", "Limb: "}), action
